...
export const sendMessage = authHandler(
  ...
  async (req: Request, res: Response) => {
    ...
    const type = "response";
    const results = await Attachment.insertMany(attachmentCreates);
    attachments = results.map((result) => result._doc);
    ...
    attachments = attachments.map((att) => ({ ...att, id: att._id }));
    message = { ...message, id: message._id, attachments: [...attachments] };
